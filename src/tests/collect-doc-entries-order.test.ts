 ...
 * the `routes/shared.ts` adapter that applies it, driven by a shuffled fake
 * collection.
 *
 * `sortBySlug` is Astro-free, so it is imported directly. `routes/shared.ts`
...
const { collectionEntries } = vi.hoisted(() => ({
  collectionEntries: [] as Array<{ id: string; data: Record<string, unknown> }>,
}));
vi.mock('astro:content', () => ({
  getCollection: async () => collectionEntries,
}));

const { collectDocEntries } = await import('../lib/routes/shared.js');
